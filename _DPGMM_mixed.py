...
class DPGMM_mixed(DPGMM_base):
    ...
    random_covariance_prior :
        ...
        a diagonal matrix with value 50.
    
    random_degrees_prior : 
        The prior of the degress of freedom parameter of the random
        effect distributions (Wishart). If it is None, it's set to 1.0.
    ...
    def _check_random_parameters(self):
        ...
        if self.random_covariance_prior is None:
            self.random_covariance_prior_ = 50*np.eye(self.n_features2)     
        else:
            ...
            _check_shape(
                ...
                "%s random_covariance_prior" % self.covariance_type,
            )
            
        if self.random_degrees_prior is None:
            self.random_degrees_prior_ = 1
        elif self.random_degrees_prior > 1 :
            self.random_degrees_prior_ = self.random_degrees_prior
        else:
            raise ValueError(
                "The parameter 'random_degrees_prior' "
                "should be greater than %d, but got %.3f."
                % (1, self.random_degrees_prior)
            )
    ...
    def _initialize(self, X, y, counts, resp):
        ...
        self.mu = np.zeros((n_subgroups, self.n_features2, )) # Random Initialization
        self.Sigma = np.zeros((n_subgroups, self.n_features2, self.n_features2)) 
        self.Sigma[:] = np.eye(self.n_features2) # Random Initialization
        
        nk, nk2, xk, sk, tk, trace = self._estimate_gaussian_parameters(
        ...
        self._estimate_precisions(nk, xk, tk, trace)
        self._estimate_scale()
        self._estimate_wishart(resp, nk2)
    ...
    def _estimate_precisions(self, nk, xk, tk, trace):
    ...
    def _estimate_wishart(self, resp, nk2):
        ...
        for k in range(self.n_components):
            ...
            self.wishart_covariance_[k] = np.linalg.inv(temp + np.linalg.inv(self.random_covariance_prior_))        
