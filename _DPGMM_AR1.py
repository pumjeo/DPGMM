...
def _expectation_corr(varpi, vartheta):
    alpha = (-1-varpi)/np.sqrt(vartheta)
    beta = (1-varpi)/np.sqrt(vartheta)
    temp_1 = (norm.pdf(alpha)-norm.pdf(beta))/(norm.cdf(beta)-norm.cdf(alpha))
    temp_2 = 1 + (alpha * norm.pdf(alpha)- beta * norm.pdf(beta))/(norm.cdf(beta)-norm.cdf(alpha))
...
class DPGMM_AR1(DPGMM_base):
    ...
    corr_precision_prior : 
        ...
        distributions (Truncated Normal). If it is None, it's set to '0.001'.
    ...
    def _check_precisions_parameters(self):
        ...
        if self.precision_shape_prior is None:
            self.precision_shape_prior_ = 0.001 
        ...
        if self.precision_rate_prior is None:
            self.precision_rate_prior_ = 0.001 
    ...
    def _check_corr_parameters(self):
        ...
        if self.corr_precision_prior is None:
            self.corr_precision_prior_ = 0.001
        elif self.corr_precision_prior > 0:
            self.corr_precision_prior_ = corr_precision_prior
        else:
            raise ValueError(
    ...
    def _initialize(self, X, y, counts, resp):
        ...
        self.varpi_ = np.repeat(0, n_subgroups) # Random Initialization
        self.vartheta_ = np.repeat(1, n_subgroups) # Random Initialization
    ...
    def _estimate_corr(self, X, y, counts, resp):
        ...
        tr_theta_split = np.split(tr_theta, np.cumsum(counts-1)[:-1], axis=0)
        tr_theta_res = np.vstack([np.sum(ele, axis=0) for ele in tr_theta_split])
        tr_pi_split = np.split(tr_pi, np.cumsum(counts-1)[:-1], axis=0)
        tr_pi_res = np.vstack([np.sum(ele, axis=0) for ele in tr_pi_split])
        error_theta_split = np.split(error_theta, np.cumsum(counts-1)[:-1], axis=0)   
        error_theta_res = np.vstack([np.sum(ele, axis=0) for ele in error_theta_split])
        error_pi_split = np.split(error_pi, np.cumsum(counts-1)[:-1], axis=0)  
        error_pi_res = np.vstack([np.sum(ele, axis=0) for ele in error_pi_split])

        self.vartheta_ = 1/(self.corr_precision_prior_ + np.sum(resp * ( (self.precision_shape_/self.precision_rate_)[np.newaxis, :] 
                             * error_theta_res + tr_theta_res ), axis=1))
        self.varpi_ = self.vartheta_ * np.sum(resp * ( (self.precision_shape_/self.precision_rate_)[np.newaxis, :] 
                                                    * error_pi_res + tr_pi_res ), axis=1)           
    ...
    def _estimate_gaussian_parameters(self, X, y, counts, resp):
        ...
        zeta_mean_duple_one = np.repeat(zeta_mean, counts-1)
        zeta_squared_mean_duple_one = np.repeat(zeta_squared_mean, counts-1)
        ...
        for k in range(self.n_components):
            xk[k] = (
                     ...
                     - np.sum(resp_duple_one[:,k][:,np.newaxis] * zeta_mean_duple_one[:,np.newaxis] 
                              * X_mi * y_1[:,np.newaxis], axis=0)
                     - np.sum(resp_duple_one[:,k][:,np.newaxis] * zeta_mean_duple_one[:,np.newaxis] 
                              * X_1 * y_mi[:,np.newaxis], axis=0)
                     + np.sum(resp_duple_one[:,k][:,np.newaxis] * zeta_squared_mean_duple_one[:,np.newaxis] 
                              * X_mi * y_mi[:,np.newaxis], axis=0)
                     )
        ...
        for k in range(self.n_components): 
            sk[k] = (
                     ...
                     - np.dot(resp_duple_one[:, k] * zeta_mean_duple_one * X_mi.T, X_1)
                     - np.dot(resp_duple_one[:, k] * zeta_mean_duple_one * X_1.T, X_mi)
                     + np.dot(resp_duple_one[:, k] * zeta_squared_mean_duple_one * X_mi.T, X_mi)
                     )
            sk[k].flat[:: n_features + 1] += self.reg_covar 

        tk = (
              np.dot(resp_duple_one.T, y_1**2)
              - 2 * np.dot( (resp_duple_one * zeta_mean_duple_one[:, np.newaxis]).T, y_mi*y_1 )
              + np.dot( (resp_duple_one * zeta_squared_mean_duple_one[:, np.newaxis]).T, y_mi**2 )
              )
    ...
    def _estimate_log_prob(self, X, y, counts):
        ...
        zeta_mean, zeta_squared_mean = _expectation_corr(self.varpi_, self.vartheta_)
        zeta_mean_duple = np.repeat(zeta_mean, counts-1)
        zeta_squared_mean_duple = np.repeat(zeta_squared_mean, counts-1)        
        ...
        for k in range(self.n_components): 
            first = (self.precision_shape_[k] / self.precision_rate_[k]) * (
                                     (y_1-np.dot(X_1, self.beta_star_mean_[k]))**2 + 
                                      zeta_squared_mean_duple * (y_mi - np.dot(X_mi, self.beta_star_mean_[k]))**2 
                                      - 2 * zeta_mean_duple * (y_1 - np.dot(X_1, self.beta_star_mean_[k])) 
                                          * (y_mi - np.dot(X_mi, self.beta_star_mean_[k]))
                                     )
            second = (
                      np.einsum('ij,ij->i', X_1, np.dot(X_1, self.beta_covariance_[k])) 
                      - 2 * zeta_mean_duple * np.einsum('ij,ij->i', X_mi, np.dot(X_1, self.beta_covariance_[k])) 
                      + zeta_squared_mean_duple * np.einsum('ij,ij->i', X_mi, np.dot(X_mi, self.beta_covariance_[k])) 
                     )
